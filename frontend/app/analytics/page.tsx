...
function Tag({ tag }: { tag: string }) {
  const s: Record<string, string> = {
    Weak:     "text-[#FF4D6D] border border-[#FF4D6D]/30",
    Building: "text-[#F5A623] border border-[#F5A623]/30",
    Good:     "text-[#39FF6A] border border-[#39FF6A]/30",
  }
...
function Gauge({ value }: { value: number }) {
  ...
  const color = value >= 70 ? "#39FF6A" : value >= 40 ? "#F5A623" : "#FF4D6D"
  return (
    <svg width="140" height="140" viewBox="0 0 140 140" className="drop-shadow-[0_0_15px_rgba(57,255,106,0.2)]">
    ...
    <AppShell>
      <div className="space-y-6">
        <div>
          ...
          <h1 className="font-serif font-black text-3xl text-white">Analytics</h1>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((s, i) => (
            <div key={s.label} className={`neo-card p-5 border border-[rgba(255,255,255,0.14)] rounded-none group/stat ${s.stripe}`} style={{ animationDelay: `${0.1 * i}s` }}>
              <p className="section-label text-[10px] mb-3 transition-colors group-hover/stat:text-white">{s.label}</p>
              <p className="font-mono text-3xl font-black text-[#E8E8F0] transition-transform group-hover/stat:scale-105 origin-left duration-300">
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          <div className="space-y-5">
            ...
            <div className="neo-card neo-card-green p-6 flex flex-col items-center border border-[rgba(255,255,255,0.14)] rounded-none">
            ...
            <div className="neo-card neo-card-pink p-6 flex flex-col border border-[rgba(255,255,255,0.14)] rounded-none relative overflow-hidden group/pred">
              <div className="absolute inset-0 pointer-events-none opacity-20 z-0" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, rgba(255,77,109,0.3) 1px, transparent 0)', backgroundSize: '32px 32px' }}></div>
              ...
              {predictionLoading ? (
                ...
                <div className="border border-[rgba(255,255,255,0.14)] bg-[rgba(255,255,255,0.05)] p-8 rounded-none text-center backdrop-blur-sm">
                  <p className="text-xs text-[#8888A0] font-mono font-bold">Not enough quiz data yet</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {/* Score and Badge Display */}
                  <div className="flex flex-col items-center justify-center py-6 border border-[rgba(255,255,255,0.14)] bg-[rgba(0,0,0,0.4)] rounded-none transition-transform duration-300 group-hover/pred:scale-[1.02] shadow-[inset_0_0_20px_rgba(255,77,109,0.1)]">
                    <div className="text-center">
                      <span className="font-serif font-black text-7xl text-white drop-shadow-[0_0_15px_rgba(255,77,109,0.4)]">{Math.round(prediction.predicted_score)}</span>
                    ...
                    <div className="mt-2">
                      <span className={cn(
                        ...
                        prediction.confidence_level === "High" ? "bg-[#39FF6A]" :
                        prediction.confidence_level === "Medium" ? "bg-[#F5A623]" : "bg-[#FF4D6D]"
                      )}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left font-mono text-[10px] text-[#E8E8F0] border-collapse">
                        <thead>
                        ...
                        <tbody className="divide-y divide-[rgba(255,255,255,0.06)]">
                          {prediction.topic_breakdown.map((row) => (
                            <tr key={row.topic} className="hover:bg-[rgba(255,255,255,0.04)] transition-colors group/row cursor-default">
                              <td className="py-2.5 pr-2 font-medium truncate max-w-[120px] transition-colors group-hover/row:text-white">{row.topic}</td>
                              ...
                              <td className="py-2.5 text-right font-black text-[#FF4D6D] group-hover/row:scale-110 origin-right transition-transform">{(row.score * 100).toFixed(0)}%</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    <div className="space-y-3 pt-4 border-t border-[rgba(255,255,255,0.14)]">
                      {prediction.topic_breakdown.map((row) => (
                        <div key={`bar-${row.topic}`} className="space-y-1.5 group/bar">
                          <div className="flex justify-between text-[8px] font-mono text-[#8888A0]">
                            <span className="truncate max-w-[150px] group-hover/bar:text-white transition-colors">{row.topic}</span>
                            <span className="font-bold text-white transition-all group-hover/bar:scale-110 origin-right">{(row.score * 100).toFixed(0)}% predicted</span>
                          </div>
                          <div className="h-1.5 bg-[rgba(255,255,255,0.05)] overflow-hidden border border-[rgba(255,255,255,0.1)] rounded-none group-hover/bar:border-[rgba(255,255,255,0.2)] transition-colors">
                            <div className="h-full transition-all duration-500 ease-out rounded-none relative" style={{
                              width: `${row.score * 100}%`,
                              backgroundColor: row.score < 0.5 ? "#FF4D6D" : row.score < 0.7 ? "#F5A623" : "#39FF6A"
                            }}>
                              <div className="absolute inset-0 opacity-0 group-hover/bar:opacity-100 transition-opacity" style={{ boxShadow: `0 0 8px ${row.score < 0.5 ? "#FF4D6D" : row.score < 0.7 ? "#F5A623" : "#39FF6A"}` }}></div>
                            </div>
                          </div>
                        </div>
                      ))}
          ...
          <div className="neo-card neo-card-amber p-6 border border-[rgba(255,255,255,0.14)] rounded-none">
                ...
                {data.topic_performance.map(t => (
                  <div key={t.topic} className="space-y-1.5 group/perf hover:translate-x-1 transition-transform duration-300">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-xs text-[#A0A0C0] group-hover/perf:text-white truncate flex-1 font-mono transition-colors">{t.topic}</span>
                      <div className="group-hover/perf:scale-105 transition-transform"><Tag tag={t.tag} /></div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-1.5 bg-[rgba(255,255,255,0.05)] overflow-hidden border border-[rgba(255,255,255,0.1)] rounded-none group-hover/perf:border-[rgba(255,255,255,0.2)] transition-colors">
                        <div className="h-full transition-all duration-500 ease-out rounded-none relative" style={{
                          width: `${t.score * 100}%`,
                          backgroundColor: t.score < 0.5 ? "#FF4D6D" : t.score < 0.7 ? "#F5A623" : "#39FF6A"
                        }}>
                          <div className="absolute inset-0 opacity-0 group-hover/perf:opacity-100 transition-opacity" style={{ boxShadow: `0 0 8px ${t.score < 0.5 ? "#FF4D6D" : t.score < 0.7 ? "#F5A623" : "#39FF6A"}` }}></div>
                        </div>
                      </div>
                      <span className="font-mono text-[10px] text-[#8888A0] w-24 text-right flex-shrink-0 group-hover/perf:text-[#39FF6A] transition-colors">
